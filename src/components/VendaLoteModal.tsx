...
export default function VendaLoteModal({ isOpen, onClose, onSuccess, parceiros, clientes, cartoes, usuarios }: Props) {
  ...
  const fetchComprasDoParceiro = async (id: string) => {
    ...
    try {
      ...
      // Distribuir saldo real do estoque entre compras — LIFO (mais recentes primeiro).
      // Compras mais recentes têm prioridade; compras antigas já vendidas ficam com saldo 0 e somem.
      const saldoRestantePorPrograma: Record<string, number> = { ...saldoEstoqueMap };

      const comprasOrdenadas = [...(comprasRes.data || [])].sort(
        (a: any, b: any) => new Date(b.created_at || b.data_entrada).getTime() - new Date(a.created_at || a.data_entrada).getTime()
      );
